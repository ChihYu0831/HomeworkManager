...
namespace HomeworkManager.Forms.Pages
{
    public class HomeworkPage : Panel
    {
        ...
        private static readonly string[] Courses = { "羽球入門", "作業系統概論", "組合語言與計算機組織", "程式設計（二）", "視窗程式設計（二）", "演算法概論", "機率與統計", "醫療資訊學概論" };
        ...
        private void Build()
        {
            ...
            // Row 1
            grpInput.Controls.Add(MakeLabel("課程名稱：", new Point(14, 34)));
            cmbCourse = new ComboBox { Location = new Point(90, 32), Size = new Size(200, 28), DropDownStyle = ComboBoxStyle.DropDown, BackColor = MainShell.ThemePanel, ForeColor = MainShell.ThemeFore };
            cmbCourse.Items.AddRange(Courses);
            grpInput.Controls.Add(cmbCourse);

            grpInput.Controls.Add(MakeLabel("作業標題：", new Point(310, 34)));
            txtTitle = new TextBox { Location = new Point(386, 32), Size = new Size(320, 28), BackColor = MainShell.ThemePanel, ForeColor = MainShell.ThemeFore };
            grpInput.Controls.Add(txtTitle);

            grpInput.Controls.Add(MakeLabel("截止日期：", new Point(726, 34)));
            dtpDue = new DateTimePicker { Location = new Point(802, 32), Size = new Size(150, 28), Format = DateTimePickerFormat.Short, MinDate = DateTime.Today };
            grpInput.Controls.Add(dtpDue);

            // Row 2
            grpInput.Controls.Add(MakeLabel("作業內容：", new Point(14, 74)));
            txtContent = new TextBox { Location = new Point(90, 72), Size = new Size(750, 68), Multiline = true, ScrollBars = ScrollBars.Vertical, BackColor = MainShell.ThemePanel, ForeColor = MainShell.ThemeFore };
            grpInput.Controls.Add(txtContent);

            grpInput.Controls.Add(MakeLabel("狀　　態：", new Point(858, 34)));
            cmbStatus = new ComboBox { Location = new Point(934, 32), Size = new Size(110, 28), DropDownStyle = ComboBoxStyle.DropDownList, BackColor = MainShell.ThemePanel, ForeColor = MainShell.ThemeFore };
            cmbStatus.Items.AddRange(new object[] { "未完成", "已完成" });
            cmbStatus.SelectedIndex = 0;
            grpInput.Controls.Add(cmbStatus);

            // Row 3 – Buttons
            var pnlBtn = new Panel { Location = new Point(90, 152), Size = new Size(700, 40), BackColor = Color.Transparent };
            pnlBtn.Controls.Add(MakeBtn("➕ 新增", Color.FromArgb(40, 167, 69), 0, BtnAdd_Click));
            pnlBtn.Controls.Add(MakeBtn("✏️ 修改", Color.FromArgb(0, 123, 255), 118, BtnUpdate_Click));
            pnlBtn.Controls.Add(MakeBtn("🗑️ 刪除", Color.FromArgb(210, 55, 55), 236, BtnDelete_Click));
            pnlBtn.Controls.Add(MakeBtn("✅ 標記完成", Color.FromArgb(102, 16, 242), 354, BtnComplete_Click));
            pnlBtn.Controls.Add(MakeBtn("🔄 清除", Color.FromArgb(108, 117, 125), 488, BtnClear_Click));
            ...
            grpSearch.Controls.Add(MakeLabel("搜尋：", new Point(14, 26)));
            txtSearch = new TextBox { Location = new Point(66, 24), Size = new Size(280, 28), BackColor = MainShell.ThemePanel, ForeColor = MainShell.ThemeFore };
            txtSearch.KeyDown += (s, e) => { if (e.KeyCode == Keys.Enter) DoSearch(); };
            grpSearch.Controls.Add(txtSearch);
            grpSearch.Controls.Add(MakeBtn("搜尋", Color.FromArgb(0, 123, 255), 354, (s, e) => DoSearch()));
            grpSearch.Controls.Add(MakeBtn("顯示全部", Color.FromArgb(108, 117, 125), 472, (s, e) => { txtSearch.Clear(); RefreshGrid(_service.GetAll()); }));
            ...
            dgv.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "課程", DataPropertyName = "CourseName", Width = 150 });
            dgv.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "標題", DataPropertyName = "Title", Width = 160 });
            dgv.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "內容", DataPropertyName = "Content", Width = 180 });
        ...
        private Label MakeLabel(string text, Point loc)
            => new Label { Text = text, Location = loc, Size = new Size(80, 24), TextAlign = ContentAlignment.MiddleRight, ForeColor = MainShell.ThemeFore, Font = new Font("微軟正黑體", 10F) };

        private Button MakeBtn(string text, Color bg, int x, EventHandler click)
        {
            var btn = new Button
            {
                Text = text,
        ...
        private void Dgv_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (row.DataBoundItem is Homework hw)
            {
                ...
                if (hw.IsCompleted)
                    ...
                    bg = dark ? Color.FromArgb(75, 55, 15) : Color.FromArgb(255, 238, 210);
                else
